...
public class DownloadingAdapter extends ArrayAdapter<DownloadTask> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.mTitle.setText(task.fileName);
        /*holder.mSize.setText(formatSize(task.finishedSize, task.totalSize));

        if(URLUtil.isHttpUrl(task.thumbnail)){
            holder.mThumbnail.setImageBitmap(getBitMapFromUrl(task.thumbnail));
        } else if (URLUtil.isFileUrl(task.thumbnail)){
            holder.mThumbnail.setImageBitmap(BitmapFactory.decodeFile(task.thumbnail.substring(8)));
        } else if (URLUtil.isAssetUrl(task.thumbnail)){
            holder.mThumbnail.setImageBitmap(getBitmapFromAsset(task.thumbnail.substring(22)));
        }
        // ImageUtil.loadImage(holder.mIcon, task.getThumbnail());

        if (task.percent > 0) {
            holder.mProgressBar.setProgress(task.percent);
        }*/

        switch (mTaskList.get(position).downloadState) {
